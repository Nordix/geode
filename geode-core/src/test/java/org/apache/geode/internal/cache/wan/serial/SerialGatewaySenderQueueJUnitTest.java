...
public class SerialGatewaySenderQueueJUnitTest {
  ...
  public void setup() {
    ...
    when(mockInternalDistributedSystem.getClock().cacheTimeMillis()).thenReturn(0L);
    ...
    cache.getInternalDistributedSystem().getClock().cacheTimeMillis();
    ...
    sender = mock(AbstractGatewaySender.class, RETURNS_DEEP_STUBS);
    ...
    when(sender.getCache().createVMRegion(any(), any())).thenReturn(region);
    when(sender.getCache().getRegion(any())).thenReturn(region);
  ...
  private class TestableSerialGatewaySenderQueue extends SerialGatewaySenderQueue {
    ...
    @Override
    protected void initializeRegion(AbstractGatewaySender sender, CacheListener listener) {
      this.region = sender.getCache().getRegion(regionName);
    }
