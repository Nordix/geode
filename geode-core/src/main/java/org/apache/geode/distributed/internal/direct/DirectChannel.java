...
public class DirectChannel {
  ...
  private int sendToMany(final Membership mgr,
    ...
    if (!directReply) {
      ...
      try {
        ...
        this.conduit.getCancelCriterion().checkCancelInProgress(ie);
        Thread.currentThread().interrupt();
      }
      for (Iterator it = totalSentCons.iterator(); it.hasNext();) {
        Connection con = (Connection) it.next();
        if (con.isClosing()) {
          bytesWritten =
              resendMessage(mgr, con.getRemoteAddress(), msg, ackWaitThreshold, ackSAThreshold);
        }
      }
    }
    */
  ...
  public int resendMessage(final Membership mgr,
      InternalDistributedMember p_destination,
      final DistributionMessage msg, long ackWaitThreshold, long ackSAThreshold)
      throws ConnectExceptions, NotSerializableException {
    InternalDistributedMember destinations[] = new InternalDistributedMember[1];
    destinations[0] = p_destination;
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("Resending ({}) to {} peers ({}) via tcp/ip",
          msg, destinations.length, Arrays.toString(destinations));
    }
    ...
    try {
      do {
        ...
        final BaseMsgStreamer ms =
            MsgStreamer.create(cons, msg, false, stats, getConduit().getBufferPool());
        try {
          startTime = 0;
          if (ackTimeout > 0) {
            startTime = System.currentTimeMillis();
